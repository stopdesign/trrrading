...
class MarketCalendar:
    ...
    def get_grid(self, exchange):
        ...
        rth = mcal.date_range(schedule, "1T", closed="left", force_close=1)
        ...
        eth = mcal.date_range(schedule, "1T", closed="left", force_close=1)

        return set(rth.view("int64") // 10**9), set(eth.view("int64") // 10**9)

    def is_rth(self, symbol, dt) -> bool:
        ex = IBKR_TO_MCAL[symbol.split(".")[1]]
        return dt_to_ts(dt.replace(second=0, microsecond=0)) in self.grids[ex][0]

    def is_open(self, symbol, dt) -> bool:
        ex = IBKR_TO_MCAL[symbol.split(".")[1]]
        return dt_to_ts(dt.replace(second=0, microsecond=0)) in self.grids[ex][1]
